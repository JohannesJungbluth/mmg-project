...
{
	// @TODO: implement visualization for color images
	int x, y, k, cx, cy ;
	...
	for (y = 0 ; y < (signed)hog_array_height ; ++y) {
		for (x = 0 ; x < (signed)hog_array_width ; ++x) {
			...
			for (k = 0 ; k < (signed)num_orientations ; ++k) {
				...
				weight > maxWeight ? maxWeight = weight : maxWeight = maxWeight;
				weight < minWeight ? minWeight = weight : minWeight = minWeight;
...

			{
				...
				for (cy = 0 ; cy < (signed)glyph_size ; ++cy) {
					for (cx = 0 ; cx < (signed)glyph_size ; ++cx) {
						...
						int min = 0;
						maxWeight < value ? min = maxWeight : min = value;
						int max = 0;
						minWeight > min ? max = minWeight : max = min;
						*glyphImage++ = max;
					}
					glyphImage += (hog_array_width - 1) * glyph_size;
				}
